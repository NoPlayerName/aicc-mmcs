...
class InputTemptTapping extends Component
{
    ...
    public function loadData()
    {
        if ($this->edit) {
            ...
            $data = app(MaterialUseAceService::class)->getTempTapping($this->chargeId);
    ...
    public function save()
    {
        ...
        if ($this->edit) {
            $save =  app(MaterialUseAceService::class)->updateTemptTapping($this->dataTapping);
            if ($save) {
                $this->loadData();
            ...
            $save =  app(MaterialUseAceService::class)->saveTemptTapping($this->dataTapping);
            if ($save) {
                $this->reset('dataTapping');
                // $this->dispatch('saved');
    ...
    {
        return view('livewire.ace.input-tempt-tapping');
    }
}
