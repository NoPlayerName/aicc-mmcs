        ...
        <div class="row">
            <div class="col-12">
                <div class="page-title-box d-flex align-items-center justify-content-between">
                    <h4 class="mb-0 font-size-18 text-uppercase">ACE Ladle Transfer Adjust</h4>
                    <div class="page-title-right">
                        <ol class="breadcrumb m-0 small">
                            <li class="breadcrumb-item"><a href="javascript: void(0);">ACE</a></li>
                            <li class="breadcrumb-item active">Ladle Transfer Adjust</li>
                        </ol>
                    </div>
                </div>
            </div>
        ...
        <div class="card shadow-sm mb-4 border-0">
            <div class="card-body">
                <div class="row align-items-end">
                    ...
                    <div class="col-md-4">
                        <label class="font-weight-bold">Material (Inoculant)</label>
                        <div wire:ignore>
                            <select class="form-control @error('material_id') is-invalid @enderror"
                                id="ladle-tf-adjust-material-select2">
                                <option value="">Select Material</option>
                                @foreach ($materials as $material)
                                <option value="{{ $material['material_code'] }}"
                                    @selected($material_id===$material['material_code'])>
                    ...
                    <div class="col-md-2">
                        ...
                        <input type="number" step="any" class="form-control @error('qty_adjust') is-invalid @enderror"
                ...
                <div class="table-responsive">
                    <table class="table table-hover table-centered mb-0">
                        ...
                        <tbody>
                            ...
                            @empty
                            <tr>
                                <td colspan="5" class="text-center py-4 text-muted">Belum ada draft adjust.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
...
@push('scripts')
<script>
    function initLadleTfAdjustMaterialSelect2() {
        const $select = $('#ladle-tf-adjust-material-select2');
        ...
        $select.select2({
            ...
            allowClear: true
        }).off('change.ladleTfAdjust').on('change.ladleTfAdjust', function () {
            Livewire.dispatch('ladleTfAdjustMaterialSelected', { materialId: $(this).val() || '' });
        });
    }

    $(document).on('livewire:navigated', function () {
        initLadleTfAdjustMaterialSelect2();

        if (!window.__ladleTfAdjustSelectBound) {
            window.__ladleTfAdjustSelectBound = true;
            Livewire.on('resetLadleTfAdjustMaterialSelect', () => {
                const $select = $('#ladle-tf-adjust-material-select2');
                if ($select.length) {
